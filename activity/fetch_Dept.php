...
if (!isset($_GET['BranchID'])) {
    die(json_encode(["statusCode" => 0, "status" => false, "errorMsg" => "Missing parameter: BranchID"]));
}

try {
    $branchID = (int)$_GET['BranchID'];

    $stmt = mysqli_prepare($link, "SELECT id, title FROM departments WHERE branch_id = ? AND status_id = 1");
